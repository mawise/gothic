...
# http://2.bp.blogspot.com/-MzlQox_nAqQ/UWBctGzafwI/AAAAAAAABaM/F-nJLG3sG4g/s1600/Plain+Gothic.jpg
  ...
  def b
    down_stroke(bot: 1, top: almost, asc: 4) +  
  ...
  def h
    down_stroke(bot: almost, top: almost, asc: 4) +  
    down_stroke(col: 2, top: 1, bot: almost)
  ensure
    self.x_pos = xp + 4*nw
  end

  def i
    down_stroke(top: almost, bot: almost) +
    dot(col: -1*almost, top: -1)
  ensure
    self.x_pos = xp + 2*nw
  end
  ...
  def k
    dot(col: 1, top: 1) +
    down_stroke(bot: almost, top: almost, asc: 4) +  
    down_stroke(col: 2, top: 1, bot: almost, asc: -2)
  ensure
    self.x_pos = xp + 4*nw
  end
  ...
  def m
    down_stroke(bot: almost, top: 1, left: almost) +  
    down_stroke(col: 2, top: 1, bot: almost) +
    down_stroke(col: 4, top: 1, bot: almost)
  ensure
    self.x_pos = xp + 6*nw
  end
  ...
  def p
    down_stroke(top: 1, bot: 1, dsc: 4, left: almost) +  
    down_stroke(col: 2, top: 1, dsc: -1) +
  ...
  def q
    down_stroke(bot: 1, asc: -1) +  
    down_stroke(col: 2, top: 1, bot: almost, dsc: 4)
  ensure
    self.x_pos = xp + 4*nw
  end

  def r
    down_stroke(top: 1, bot: 1, left: almost, right: almost) +
    dot(col: 1, top: 1)
  ensure
    self.x_pos = xp + 3*nw
  end
  ...
  def u
    down_stroke(bot: 1, top: almost) +  
    down_stroke(col: 2, top: almost, bot: 1, right: almost)
  ensure
    self.x_pos = xp + 4*nw
  end
  ...
  def v
    down_stroke(bot: 1, top: almost) +  
    down_stroke(col: 2, top: almost, dsc: -1)
  ensure
    self.x_pos = xp + 4*nw
  end
  ...
  def w
    down_stroke(bot: 1, top: almost) +  
    down_stroke(col: 2, bot: 1, top: almost) +  
    down_stroke(col: 4, top: almost, dsc: -1)
  ensure
    self.x_pos = xp + 6*nw
  end

  def x
    down_stroke(top: 1, bot: 1, left: almost) +
    dot(col:almost, top: 1) +
    dot(top: 3.5, col: -1, wide: 2.5, tall: 0) # cross
  ensure
    self.x_pos = xp + 4*nw
  end

  def y
    down_stroke(top: almost,  bot: 1) +
    down_stroke(col: 2, top: almost, dsc: 3) +
    bottom_dot(top: 3)
  ensure
    self.x_pos = xp + 4*nw
  end
